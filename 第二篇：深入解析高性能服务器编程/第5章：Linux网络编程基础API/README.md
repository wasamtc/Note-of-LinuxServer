...
这个sockatmark很坑的，如果没setsocket选项，那么recv会停在OOB字节和下一字节，最坑的是读下一字节时还是返回1，不过这时不是OOB字节，如果还是用MSG_OOB去读的话就会出错返回-1.如果setsocket选项，那么只停在OOB字节，且只有OOB字节那里返回1。

具体见链接

[带外数据：TCP紧急模式分析](https://www.its203.com/article/aprilweet/62421995)
